...
func PlaybackControls(client *gumble.Client, message string, isPrivate bool, sender string) bool {
	...
	if isCommand(message, "time") {
		dt := time.Now()
		helper.MsgDispatch(isPrivate, sender, client, dt.Format("01-02-2006 15:04:05 Monday"))
		return true
	}

	// Skip to next track in playlist
	if isCommand(message, "skip") || isCommand(message, "next") {
...
func SearchCommands(client *gumble.Client, message string, isPrivate bool, sender string) bool {
	...
	if isCommand(message, "music") {
		results := search.ShowFullList()
		output := ""
		for _, value := range results {
			output = output + "<br />" + value
		}
		helper.MsgDispatch(isPrivate, sender, client, output)
		return true
	}
